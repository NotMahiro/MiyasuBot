...
async function getApproval(player, message) {
    return new Promise(async (res, rej) => {
        try {
            ...
            let msg = await channel.send({ components: [new MessageActionRow().addComponents([new MessageButton().setCustomId("1_tic_tac_toe_choose").setStyle("SUCCESS").setEmoji("✔").setLabel("Accept"), new MessageButton().setCustomId("2_tic_tac_toe_choose").setStyle("DANGER").setEmoji("❌").setLabel("Reject")])], embeds: [{ color: "DARK_NAVY", title: this.requestTitle, fields: [{ name: "Sender", value: message.author.username, inline: true }, { name: "Server", value: message.guild.name, inline: true }, { name: "Channel", value: message.channel.toString(), inline: true }] }] });
