...
class TicTacToe {
    ...
    */
    ...
    async solo(message, bot) {
        ...
        const sent = await message.channel.send({ components: row, embeds: [{ color: "DARK_VIVID_PINK", title: this.startTitle, description: getDescription(user, botc) }] })
    ...
    async duo(message, player2) {
        ...
        const sent = await message.channel.send({ components: row, embeds: [{ color: "DARK_VIVID_PINK", title: this.startTitle, description: getDescription(player1Choice, player2Choice) }] })
        ...
        for (let i = 1; options.length !== 0; i++) {
            ...
            let rowss = await getComponents(options);
            sent.edit({ components: rowss, embeds: [{ color: "DARK_VIVID_PINK", title: this.startTitle, description: getDescription(player1Choice, player2Choice) }] })
